/**
 * Copyright 2026 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

// Do not modify this file by hand!
// Re-generate this file by running lit-localize

import {str} from '@lit/localize';

/* eslint-disable no-irregular-whitespace */
/* eslint-disable @typescript-eslint/no-explicit-any */

export const templates = {
  s00f485d30c4d7069: '‏‮Disable‬‏ ‏‮initial‬‏ ‏‮load‬‏',
  s02c64e6a6306c0b0: '‏‮Add‬‏ ‏‮slot‬‏',
  s03b4c3722970eac8: '‏‮Continue‬‏ ‏‮reading‬‏',
  s03c563952f49581d: str`${0} ‏‮prompt‬‏ ‏‮was‬‏ ‏‮dismissed‬‏.`,
  s0536dee760be4bf2: '// ‏‮Configure‬‏ ‏‮privacy‬‏ ‏‮settings‬‏.',
  s06d92c8a54c8a011: '‏‮Slot‬‏ ‏‮template‬‏',
  s08995feb9716ee48: '‏‮Out‬‏-‏‮of‬‏-‏‮page‬‏ ‏‮format‬‏',
  s09205907b5b56cda: '‏‮No‬‏',
  s09c694b6602d365d: '‏‮Side‬‏ ‏‮rail‬‏ ‏‮ad‬‏ (‏‮left‬‏)',
  s0a0c98fc141bc5b6:
    '‏‮Prompt‬‏ ‏‮the‬‏ ‏‮user‬‏ ‏‮for‬‏ ‏‮their‬‏ ‏‮consent‬‏ ‏‮to‬‏ ‏‮view‬‏ ‏‮the‬‏ ‏‮rewarded‬‏ ‏‮ad‬‏.',
  s0ae4a4d62b686a77: '‏‮Fetch‬‏ ‏‮margin‬‏ (% ‏‮viewport‬‏)',
  s0d576da69783c058: str`${0} ‏‮slots‬‏ ‏‮return‬‏ ‏‮null‬‏ ‏‮if‬‏ ‏‮the‬‏ ‏‮page‬‏ ‏‮or‬‏ ‏‮device‬‏ ‏‮does‬‏ ‏‮not‬‏ ‏‮support‬‏ ‏‮them‬‏.`,
  s0eb106f4b9813ad3: '‏‮Non‬‏-‏‮personalized‬‏ ‏‮ads‬‏',
  s1163c710c6c9bccf: '‏‮Gaming‬‏ ‏‮interstitial‬‏ ‏‮ad‬‏',
  s11a3c998dc1af0b1: '‏‮Sample‬‏ ‏‮ads‬‏',
  s133bb1ee15ece5ae: str`${0} ‏‮was‬‏ ‏‮closed‬‏.`,
  s1bc6849021c39575: '‏‮Render‬‏ ‏‮margin‬‏ (% ‏‮viewport‬‏)',
  s1c1f567a55f86cc2: '‏‮Unhide‬‏ ‏‮window‬‏',
  s1c67a1fa4d07bd8b: '‏‮Page‬‏ ‏‮URL‬‏',
  s1e6dc9c6efd64b15: '‏‮Disable‬‏ ‏‮ads‬‏ ‏‮personalization‬‏',
  s21e31bdc9713e7fd: '‏‮Child‬‏',
  s23dffb234cd0ff74: '‏‮Slot‬‏-‏‮level‬‏ ‏‮configuration‬‏',
  s24befdfb1b2afce1: '‏‮Teen‬‏',
  s267a52c34c200ef0: '‏‮Single‬‏ ‏‮Request‬‏ ‏‮Architecture‬‏ (‏‮SRA‬‏)',
  s292d08a9cea72454: '‏‮Fluid‬‏ ‏‮ad‬‏',
  s2a071e5a3e79454a: '‏‮Key‬‏=‏‮Value‬‏1,‏‮Value‬‏2,...',
  s2bba125a7eef0bb9: '‏‮Reward‬‏ ‏‮granted‬‏.',
  s2d647200b120ea41: '‏‮Ad‬‏ ‏‮unit‬‏ ‏‮path‬‏',
  s3132d1a0580760bc: '‏‮Child‬‏-‏‮directed‬‏ ‏‮treatment‬‏',
  s3400fb34f1e245a9: '‏‮Multi‬‏-‏‮size‬‏ ‏‮ad‬‏',
  s352f8b213acc822a: str`${0} ‏‮is‬‏ ‏‮loaded‬‏.`,
  s3870e706a2084c79: str`‏‮Invalid‬‏ ‏‮value‬‏ ‏‮for‬‏ ‏‮key‬‏ ${0}: ${1}`,
  s3aa2bb1183ed9183: '‏‮Inactivity‬‏',
  s3be169991df10a5d: '‏‮Open‬‏ ‏‮preview‬‏',
  s3e0b42b56e05829d: '‏‮Sizes‬‏',
  s3e745f11d41db048: '‏‮Age‬‏ ‏‮treatment‬‏',
  s4089274c7ce7fb00: '‏‮EU‬‏ ‏‮user‬‏ ‏‮consent‬‏',
  s4121536e35beb2ed: '‏‮Fixed‬‏-‏‮size‬‏ ‏‮ad‬‏ (728‏‮x‬‏90)',
  s417b90913e05bc17: '‏‮Preview‬‏',
  s419ce4c474f1f9a8: '// ‏‮Configure‬‏ ‏‮page‬‏-‏‮level‬‏ ‏‮settings‬‏.',
  s42278ac7b37a5245: '‏‮Add‬‏ ‏‮targeting‬‏ ‏‮key‬‏-‏‮values‬‏',
  s43e9aef7aa01fca9: '‏‮Unavailable‬‏',
  s4535f519744108bf: '‏‮Output‬‏ ‏‮format‬‏',
  s4661d2a8c1259e58: '‏‮Sample‬‏ ‏‮ads‬‏ (‏‮out‬‏-‏‮of‬‏-‏‮page‬‏)',
  s48390e2c954ba495: '‏‮Fixed‬‏-‏‮size‬‏ ‏‮ad‬‏ (100‏‮x‬‏100)',
  s4839a6cee190597f: '‏‮Lazy‬‏ ‏‮loading‬‏',
  s49b92d61c8e9e116: '‏‮Please‬‏ ‏‮specify‬‏ ‏‮a‬‏ ‏‮valid‬‏ ‏‮URL‬‏.',
  s4edecd7e589254c3: '‏‮Ad‬‏ ‏‮exclusion‬‏ ‏‮labels‬‏',
  s51791dbd45e9686e: '‏‮Backward‬‏ ‏‮navigation‬‏',
  s51e4d68633a473de: '‏‮Collapse‬‏ ‏‮ad‬‏ ‏‮slots‬‏',
  s53a96edcf36221b2: '‏‮Ad‬‏ ‏‮slot‬‏ ‏‮sizes‬‏',
  s5bf3f3d517569ebd: '‏‮Anchor‬‏ ‏‮ad‬‏ (‏‮bottom‬‏)',
  s5c64f55f3fbc34f1: str`‏‮You‬‏ ‏‮have‬‏ ‏‮been‬‏ ‏‮rewarded‬‏ ${0} ${1}!`,
  s5e13187e6961d8b1: '// ‏‮Define‬‏ ‏‮out‬‏-‏‮of‬‏-‏‮page‬‏ ‏‮slots‬‏.',
  s5e8250fb85d64c23: '‏‮Close‬‏',
  s5fc316a7f287b7be:
    '‏‮This‬‏ ‏‮sample‬‏ ‏‮cannot‬‏ ‏‮be‬‏ ‏‮previewed‬‏ ‏‮in‬‏ ‏‮an‬‏ ‏‮iframe‬‏',
  s61ee08c5b62e5b08:
    '‏‮Destroy‬‏ ‏‮the‬‏ ‏‮rewarded‬‏ ‏‮ad‬‏ ‏‮slot‬‏ ‏‮if‬‏ ‏‮the‬‏ ‏‮user‬‏ ‏‮does‬‏ ‏‮not‬‏ ‏‮consent‬‏.',
  s669b18c6d2d9c95b: '‏‮None‬‏',
  s68fe43baedc49318: '‏‮Refresh‬‏ ‏‮preview‬‏',
  s6b0627385c38c206: '‏‮Add‬‏ ‏‮an‬‏ ‏‮ad‬‏ ‏‮exclusion‬‏ ‏‮label‬‏',
  s6ead833ff5e49608: '‏‮Ad‬‏ ‏‮intents‬‏ ‏‮ad‬‏',
  s703415876ae4626f: '‏‮Fixed‬‏-‏‮size‬‏ ‏‮ad‬‏ (750‏‮x‬‏200)',
  s707a334b3b2a7140: str`${0}. ‏‮Click‬‏ ‏‮the‬‏ ‏‮button‬‏ ‏‮below‬‏ ‏‮to‬‏ ‏‮preview‬‏ ‏‮in‬‏ ‏‮a‬‏ ‏‮new‬‏ ‏‮window‬‏.`,
  s710f900ef624e363: '‏‮Anchor‬‏ ‏‮ad‬‏ (‏‮top‬‏)',
  s71e012bd812c44a2: '// ‏‮Define‬‏ ‏‮static‬‏ ‏‮ad‬‏ ‏‮slots‬‏.',
  s733564cddc33799f:
    '‏‮Error‬‏: ‏‮unable‬‏ ‏‮to‬‏ ‏‮connect‬‏ ‏‮to‬‏ ‏‮the‬‏ ‏‮GPT‬‏ ‏‮Sample‬‏ ‏‮Builder‬‏',
  s758d8c59379ea482: '‏‮Page‬‏-‏‮level‬‏ ‏‮targeting‬‏',
  s77b0113eae08368b: '‏‮Sample‬‏ ‏‮template‬‏',
  s7b5fa742d13b462e: '‏‮Slot‬‏ ‏‮settings‬‏',
  s8233e942045a2b50: str`${0} ‏‮is‬‏ ‏‮not‬‏ ‏‮supported‬‏ ‏‮on‬‏ ‏‮this‬‏ ‏‮page‬‏.`,
  s85ed67972e8ca8a4: '‏‮Require‬‏ ‏‮local‬‏ ‏‮storage‬‏ ‏‮consent‬‏',
  s863c0ad66f72f959: str`‏‮No‬‏ ‏‮ad‬‏ ‏‮returned‬‏ ‏‮for‬‏ ${0} ‏‮slot‬‏.`,
  s8a2c4a3aa651b7fb: '‏‮Remove‬‏ ‏‮slot‬‏',
  s8b97ea84af17c029: '‏‮Never‬‏',
  s8e732383cac5c9fd: '100‏‮x‬‏100,‏‮fluid‬‏,...',
  s90465cbd227840a3: str`${0} ‏‮Scroll‬‏ ‏‮page‬‏ ‏‮to‬‏ ‏‮activate‬‏.`,
  s93d1fd626db001f3: str`${0} ‏‮is‬‏ ‏‮active‬‏.`,
  s96f81f2fccc92df4:
    '// ‏‮Register‬‏ ‏‮all‬‏ ‏‮previously‬‏ ‏‮defined‬‏ ‏‮ad‬‏ ‏‮slots‬‏.',
  s9919a3c7a0754a26:
    '‏‮Labels‬‏ ‏‮must‬‏ ‏‮be‬‏ 127 ‏‮characters‬‏ ‏‮or‬‏ ‏‮less‬‏',
  s9d21580cc7643953: str`${0} ‏‮is‬‏ ‏‮loading‬‏...`,
  s9dc1289fa45bd694: '‏‮Interstitial‬‏ ‏‮triggers‬‏',
  sa2b2ab2dd02accfd: '‏‮Link‬‏ ‏‮click‬‏',
  sa6e68f4d7e8574ed: '‏‮Ad‬‏ ‏‮spacing‬‏ (% ‏‮viewport‬‏)',
  sa9c78a7a0167358f: str`‏‮Duplicate‬‏ ‏‮key‬‏: ${0}`,
  sab590a46db406ca7:
    '‏‮Please‬‏ ‏‮specify‬‏ ‏‮a‬‏ ‏‮valid‬‏ ‏‮ad‬‏ ‏‮unit‬‏ ‏‮path‬‏.',
  sb457d189f407a5a6: '‏‮Restrict‬‏ ‏‮data‬‏ ‏‮processing‬‏',
  sb4a18873f72e8f57: str`‏‮Invalid‬‏ ‏‮size‬‏: ${0}`,
  sb4fb36c094a78d99: str`‏‮Invalid‬‏ ‏‮key‬‏: ${0}`,
  sb69b079c821c34c8: '‏‮Treatments‬‏',
  sb813a33f700541bf: '‏‮Loading‬‏ ‏‮consent‬‏ ‏‮dialog‬‏...',
  sbba26a884a26c0c7: '‏‮Fixed‬‏-‏‮size‬‏ ‏‮ad‬‏ (300‏‮x‬‏250)',
  sc5c8140d066b7231: '// ‏‮Enable‬‏ ‏‮GPT‬‏ ‏‮services‬‏.',
  sc8625c36e2ee45d5:
    '‏‮Display‬‏ ‏‮the‬‏ ‏‮rewarded‬‏ ‏‮ad‬‏ ‏‮if‬‏ ‏‮the‬‏ ‏‮user‬‏ ‏‮consents‬‏.',
  sc907ffeeafb021d1: '‏‮Output‬‏ ‏‮settings‬‏',
  scb489a1a173ac3f0: '‏‮Yes‬‏',
  sccb7f7246c10ecb6: '‏‮Side‬‏ ‏‮rail‬‏ ‏‮ad‬‏ (‏‮right‬‏)',
  sd3926a53f7b06107:
    '// ‏‮Request‬‏ ‏‮and‬‏ ‏‮render‬‏ ‏‮all‬‏ ‏‮previously‬‏ ‏‮defined‬‏ ‏‮ad‬‏ ‏‮slots‬‏.',
  sd4801ff24e6e00e1: str`‏‮No‬‏ ‏‮value‬‏ ‏‮for‬‏ ‏‮key‬‏: ${0}`,
  sd59d6171f900185b: '‏‮Use‬‏ ‏‮page‬‏ ‏‮setting‬‏',
  sd5ac53cfd233dbcb: '‏‮Limited‬‏ ‏‮ads‬‏',
  sd7b3aac1efbac63c:
    '‏‮Watch‬‏ ‏‮an‬‏ ‏‮ad‬‏ ‏‮to‬‏ ‏‮receive‬‏ ‏‮a‬‏ ‏‮special‬‏ ‏‮reward‬‏?',
  sdb90b681e373be33: '‏‮Page‬‏-‏‮level‬‏ ‏‮configuration‬‏',
  sdee10f7d35f52edf: '‏‮AdSense‬‏ ‏‮attributes‬‏',
  sdf7b9d8028213f69: '‏‮Mobile‬‏ ‏‮scaling‬‏ ‏‮multiplier‬‏',
  se21652e86046e76d: '‏‮If‬‏ ‏‮no‬‏ ‏‮ad‬‏ ‏‮is‬‏ ‏‮returned‬‏',
  se53efa37a0c7e1d9: '‏‮Collapse‬‏ ‏‮ad‬‏ ‏‮slot‬‏',
  se6dd79643508bb80: '‏‮Targeting‬‏',
  se78c02e9a98a0f47: '‏‮Privacy‬‏',
  se8fba10796fd950d: '‏‮Before‬‏ ‏‮requesting‬‏ ‏‮an‬‏ ‏‮ad‬‏',
  seb61938dbbd0dd2b: '‏‮Browser‬‏ ‏‮navigation‬‏',
  secff207571282402: '‏‮Web‬‏ ‏‮interstitial‬‏ ‏‮ad‬‏',
  see7e5431ccf76ccc: '‏‮Unspecified‬‏',
  sefe480470a2c783c: '‏‮Rewarded‬‏ ‏‮ad‬‏',
  sf0cab2483f66aa2e: '‏‮Custom‬‏',
  sf7886e272836b258: str`${0} ‏‮slot‬‏ ‏‮is‬‏ ‏‮ready‬‏.`,
  sf82ca28be9ba0f11: '‏‮End‬‏ ‏‮of‬‏ ‏‮article‬‏',
  sf915707accde585c: '‏‮Under‬‏ ‏‮the‬‏ ‏‮age‬‏ ‏‮of‬‏ ‏‮consent‬‏',
  sf93d227444bc5687: '‏‮Page‬‏ ‏‮settings‬‏',
  sfe2e9ccd10b35b4a: '‏‮Force‬‏ ‏‮SafeFrame‬‏',
};
